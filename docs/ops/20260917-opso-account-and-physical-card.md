...
Implementation and enablement status for steps 1–3 is tracked in [the canonical web-account handoff](20260917-opso-canonical-web-account.md). The isolated implementation includes a dedicated HMAC client-IP sender and remains default-off pending deployment/configuration of the matching API verifier, current ingress review and actual two-client acceptance; no legacy card transfer is implied.
