...
**Son güncelleme:** 2026-09-17 (OpSo ürün sayfası ve güvenlik paketi canlıda doğrulandı)
...
- **LIVE VERIFIED:** `32075c8fa97dfb7f5b82faa0dff3f66930b5d2b3`, image `sha256:884e8e68dbc0443c534ae139a1b54678488c25585b63dbd3c83e8d942943cbd0`, 2026-09-16T23:12Z. Yalnızca `opsolid` uygulaması değişti; DB container/volume, diğer container'lar ve eski sunucu kaynak/ayar dosyaları değişmedi. `/api/health` exact commit ve `dbOk:true`; 17 dış HTTP kontrolü geçti (DE/EN/TR ürün/giriş/kayıt, eski ürün ve yeni-kart ekranı, yetkisiz API401, NFC görsel200, optimizer/source-map404). Canlı tarayıcıda renkli örnek seçimi ve tüm ilgili görseller doğrulandı. İki giriş ekranı runtime OAuth ayarını okur; gizli değişken olmadan build alınması Google düğmesini artık kalıcı gizlemez. Gerçek Google sağlayıcı giriş işlemi bu yayında tekrarlanmadı.
- **Runtime rolü VERIFIED:** uygulama artık `opsolid_runtime_20260917_32075c8` rolüyle bağlanıyor; superuser/owner/CREATEDB/CREATEROLE/BYPASSRLS/TRUNCATE yetkileri yok. RLS/PUBLIC/sahiplik/veri dönüşümü yapılmadı. Önce aynı imaj izole geri yüklemede gerçek SCRAM yanlış/doğru parola, Prisma CRUD+geri alma, gerçek keşif JSON filtresi ve 13 HTTP kontrolünü geçti. İlk prova HBA dosya yolunun restart gerektirmesi nedeniyle uygulama yaratılmadan durdu; mevcut clone dosyasının atomik değiştirilip reload edilmesiyle onarıldı. Geçici prova container'ları kaldırıldı; müşteri verisi yerel makineye kopyalanmadı.
- **Geri dönüş / sonraki deploy:** sunucuda root-only `/var/backups/opsolid/app-cutover-1a6a38b57fda46b6b8c39eac` eski imaj ve tam ayarları tutar. İncelenen `scripts/ops/opsolid-app-cutover.py` ileri hata durumunda otomatik, doğrulanmış app-only rollback uygular; 17 hata/başarı testi geçti. Kalıcı kaynak Compose ve `.env` bilerek değiştirilmedi; eski GitHub deployment workflow'u bootstrap/seed/rsync içerdiği için bu PR main'e birleştirilmemeli veya eski komutla yeniden deploy edilmemeli. Sonraki yayın güncel canlı ayarı temel almalı ve bu geçiş sözleşmesini korumalıdır. Yeni ops araçları deploy edilen uygulama kodunu değiştirmez; canlı imajın source commit'i yukarıda sabittir.
- **GitGuardian:** `37368921` yalnızca herkese açık Türkçe giriş açıklamasıydı; exact occurrence incelenip “not a secret” olarak sınıflandırıldı. Sonraki `32075c8` kontrolü yeşil.
