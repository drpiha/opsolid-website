...
interface Props {
  locale: string;
  /** Server-resolved: render the Google button only when OAuth is configured. */
  googleEnabled?: boolean;
}

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export function LoginClient({ locale, googleEnabled = false }: Props) {
  const { t } = useLocale();
  ...
  return (
    <div className="w-full">
      ...
      {/* Divider + Google — only when OAuth is configured on the server. */}
      {googleEnabled && (
        <>
