...
function LinkColumn({ title, links, labelMap }: LinkColumnProps) {
  return (
    <div>
      <h4 className="meta mb-4">{title}</h4>
      <ul className="space-y-3">
        {links.map((link) => (
          <li key={link.href}>
            <Link
              href={link.href}
              className="text-body-sm text-ink-300 hover:text-copper-200 transition-colors duration-fast ease-snap"
            >
...
export function Footer() {
  ...
  return (
    <footer
      className="relative text-ink-200"
      style={{
        background:
      ...
      <div className="container-wide py-16 md:py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.4fr_repeat(4,1fr)] gap-10 lg:gap-12">
          ...
          <div>
            <Link
              href="/"
              className="inline-flex items-center gap-2.5 text-[1.25rem] font-display font-medium leading-none text-ink-100 tracking-[-0.01em]"
            >
              <span
                aria-hidden="true"
            ...
            <p className="mt-5 max-w-xs text-body-sm text-ink-300 leading-relaxed text-pretty">
              {ft.description}
            </p>
            <div className="mt-6 flex items-center gap-2">
              {[
                ...
                <a
                  ...
                  aria-label={label}
                  className="flex h-9 w-9 items-center justify-center rounded-pill border border-line text-ink-400 hover:text-copper-200 hover:border-line-firm transition-colors duration-fast ease-snap"
                >
                  <Icon size={16} />
                </a>
              ))}
          ...
          {columns.map((col) => (
            <LinkColumn key={col.key} title={col.title} links={col.links} labelMap={linkLabels} />
          ))}
        </div>
        ...
        >
          <p className="meta">
            {ft.copyright} · <span className="text-ink-300">Hosted in Frankfurt · GDPR-native · No vendor lock-in</span>
          </p>
          <ul className="flex flex-wrap items-center gap-x-6 gap-y-2">
            {FOOTER_LINKS.legal.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className="text-body-sm text-ink-400 hover:text-ink-100 transition-colors duration-fast ease-snap"
                >
