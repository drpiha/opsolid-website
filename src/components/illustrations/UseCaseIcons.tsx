...
/**
 * Popl-style use-case icons.
 * 12 icons cycling through 3 accent tones (ink / brand red / neutral).
 * Shapes preserved verbatim; only the circle background colour and the
 * inner stroke colours were recalibrated for the new palette.
 *
 * Inner strokes use `#FFFFFF` so the linework reads clearly against any
 * of the three accent fills.
 */

const INK = "#0A0A0A";
const BRAND = "#E63946"; // brand red
const NEUTRAL = "#A3A3A3"; // neutral-400 — soft grey accent

const LINE = "#FFFFFF"; // foreground line colour on the circle
const LINE_DIM = "rgba(255,255,255,0.7)";

/** Pick a solid accent color per icon position. 3-tone cycle. */
function accentFor(index: number): string {
  const cycle = index % 3;
  if (cycle === 0) return INK;
  if (cycle === 1) return BRAND;
  return NEUTRAL;
}
