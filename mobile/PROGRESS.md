...
- **Templates locale narrowing.** The 96 v2 templates originally shipped per-template copy tables keyed on `'de' | 'en' | 'tr'`. The first M6 pass kept `TemplateProps.locale` narrow and routed `es|it|fr|ar` through a `narrowTemplateLocale()` helper that fell back to English at the call site. **Now superseded** by the per-template translation pass below.
  - **M6 follow-up — per-template copy tables [DONE — committed].** Per-template copy tables widened to 7 locales, 96 templates × 4 new locales (es/it/fr/ar). New `TemplateLocale` alias in `src/components/cards/templates/v2/types.ts` covers all 7 site locales; `TemplateProps.locale` widened to match. `narrowTemplateLocale()` kept as a passthrough for safety so existing call-sites stay valid. Each template's `Record<…, XxxCopy>` map now ships full ES/IT/FR/AR entries — translations are machine-quality dictionary substitutions (formal register: usted/Lei/vous; native Arabic script). Shared row helpers (`ContactRows`, `ExchangeSlot`, `SendMyInfoSlot`, `ExchangeButton`) widened to accept the full `Locale` and route `es|it|fr|ar` to the existing en lookup at runtime where their own copy hasn't been broken out yet (modal/order-form copy is a separate sprint). Public card visuals on `/c/<slug>?lang=es|it|fr|ar` now render localised copy on the card itself.
...
- Card-level modal copy (Exchange / Send-my-info bottom-sheet) for `es|it|fr|ar` — currently falls back to `en` at runtime; the static labels on the card itself are now localised.
